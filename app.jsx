...
<style>{`@import url('${f.url}'); *{box-sizing:border-box;margin:0;padding:0} body{background:${p.bg}} input::placeholder,textarea::placeholder{color:${p.dim}!important;opacity:1} ::-webkit-scrollbar{width:3px;height:3px} ::-webkit-scrollbar-thumb{background:${p.border};border-radius:3px} select option{background:${p.card};color:${p.text}} @keyframes blink{0%,100%{opacity:.2}50%{opacity:.05}} @keyframes dotpulse{0%,80%,100%{opacity:.15;transform:scale(.7)}40%{opacity:1;transform:scale(1)}} @keyframes tbIn{from{opacity:0;transform:translateY(4px)}to{opacity:1;transform:translateY(0)}} ${settings.customCSS || ""}`}</style>
